...
export default function Design3() {
  ...
  const [shippingInfo, setShippingInfo] = useState({ fullName:"", email:"", address:"", city:"", province:"", postalCode:"" });
  ...
  const activeCount = filters.categories.size + filters.brands.size + (filters.minRating ? 1 : 0) + (filters.dealsOnly ? 1 : 0);
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.dealsOnly) list = list.filter(p => (p.discount ?? 0) > 0 || p.tag === "Deal");

    if (sortBy === "price-low") list.sort((a, b) => a.price - b.price);
    if (sortBy === "price-high") list.sort((a, b) => b.price - a.price);
    if (sortBy === "rating") list.sort((a, b) => b.rating - a.rating);
    if (sortBy === "reviews") list.sort((a, b) => b.reviews - a.reviews);
    if (sortBy === "featured") list.sort((a, b) => (b.tag === "Deal") - (a.tag === "Deal"));
    return list;
  ...
  const dealsProducts = PRODUCTS.filter(p => (p.discount ?? 0) > 0 || p.tag === "Deal");
  const popularProducts = PRODUCTS.filter(p => p.tag === "Popular");
  ...
  function changeQty(productId, delta) {
    setCart(prev => prev.map(x => (x.productId === productId ? { ...x, qty: x.qty + delta } : x)).filter(x => x.qty > 0));
  }
  function removeFromCart(productId) { setCart(prev => prev.filter(x => x.productId !== productId)); }
  function goToDeals() { setFilters(f => ({ ...f, dealsOnly: true })); setRoute("shop"); }
    ...
    <div className="tn-shell">
      <header className="tn-nav">
        <div className="tn-nav-inner">
          ...
          <div className="search-wrap">
            <input value={searchInput} onChange={(e) => { setSearchInput(e.target.value); setRoute("shop"); }} placeholder="Search products, brands..." />
          </div>
          <nav className="menu">
            <button className={`nav-link ${route === "home" ? "active" : ""}`} onClick={() => setRoute("home")}>Home</button>
            <button className={`nav-link ${route === "shop" && !filters.dealsOnly ? "active" : ""}`} onClick={() => { setFilters(f => ({ ...f, dealsOnly: false })); setRoute("shop"); }}>Shop All</button>
            <button className={`nav-link ${route === "shop" && filters.dealsOnly ? "active" : ""}`} onClick={goToDeals}>🔥 Deals</button>
      ...
      {route === "home" && (
        <main className="page page-home">
          <section className="hero">
            <div>
              ...
              <div className="hero-actions">
                ...
                <button className="secondary-btn" onClick={goToDeals}>View Deals 🔥</button>
              </div>
            </div>
          ...
          <section className="section">
            <div className="section-head"><h2>🔥 Today's Deals</h2><button className="link-btn" onClick={goToDeals}>See all deals</button></div>
            <div className="product-grid four">{dealsProducts.slice(0, 4).map(card)}</div>
          </section>

          <section className="section">
            <div className="section-head"><h2>Most Popular</h2><button className="link-btn" onClick={() => setRoute("shop")}>View all</button></div>
            <div className="product-grid four">{popularProducts.slice(0, 4).map(card)}</div>
          </section>
        </main>
      )}
        ...
        <main className="page page-shop">
          <section className="shop-layout">
            <aside className="sidebar">
              ...
              {activeCount > 0 && (
                <button className="clear-filters" onClick={() => setFilters({ categories: new Set(), brands: new Set(), maxPrice: 2500, minRating: 0, dealsOnly: false })}>
                  ↺ Clear all filters
                </button>
              )}
              ...
              <div className="filter-group">
                ...
                {CATEGORIES.map(c => (
                  <label key={c}>
                    <input type="checkbox" checked={filters.categories.has(c)} onChange={(e) => setFilters(f => {
                      const next = new Set(f.categories); e.target.checked ? next.add(c) : next.delete(c); return { ...f, categories: next };
                    })} /> {c}
                  </label>
                ))}
              </div>
              ...
              <div className="filter-group">
                ...
                {brands.map(b => (
                  <label key={b}>
                    <input type="checkbox" checked={filters.brands.has(b)} onChange={(e) => setFilters(f => {
                      const next = new Set(f.brands); e.target.checked ? next.add(b) : next.delete(b); return { ...f, brands: next };
                    })} /> {b}
                  </label>
                ))}
              </div>
            ...
            <section className="shop-main">
              <div className="shop-top">
                <button className="filter-pill">⚙ Filters {activeCount ? <span>{activeCount}</span> : null}</button>
          ...
          <section className="product-layout">
            ...
            <div>
              ...
              <div className="buy-row">
                <div className="qty-box"><button onClick={() => setDetailQty(q => Math.max(1, q - 1))}>−</button><strong>{detailQty}</strong><button onClick={() => setDetailQty(q => q + 1)}>+</button></div>
                <button className="primary-btn wide" onClick={() => addToCart(product.id, detailQty)}>🛒 Add to Cart</button>
              </div>

              <div className="specs">
                <h3>SPECIFICATIONS</h3>
                <div className="spec-grid">{Object.entries(product.specs).map(([k, v]) => <div key={k}><small>{k}</small><b>{v}</b></div>)}</div>
              </div>
            </div>
          </section>
        </main>
      ...
      {route === "cart" && (
        <main className="page page-cart">
          ...
          <div className="steps">
            ...
            <span className={checkoutStep === "confirmed" ? "active" : ""}>✔ Confirmed</span>
          </div>
          ...
          {checkoutStep === "cart" && (
            <div className="cart-layout">
                ...
                {cartItems.length === 0 ? <p className="empty">Your cart is empty.</p> : cartItems.map(({ product, qty }) => (
                  <article className="cart-item" key={product.id}>
                    ...
                    <div className="cart-actions">
                      <div className="qty-box"><button onClick={() => changeQty(product.id, -1)}>−</button><strong>{qty}</strong><button onClick={() => changeQty(product.id, 1)}>+</button></div>
              ...
              <aside className="summary">
                ...
                <button className="primary-btn wide" disabled={!cartItems.length} onClick={() => setCheckoutStep("shipping")}>Proceed to Shipping →</button>
              </aside>
            </div>
          )}
            ...
            <div className="cart-layout">
              <section>
                ...
                <div className="filter-group" style={{ borderTop: "none", paddingTop: 0 }}>
                  ...
                  <input value={shippingInfo.fullName} onChange={(e) => setShippingInfo(s => ({ ...s, fullName: e.target.value }))} placeholder="Jane Smith" />
                  <label>Email Address *</label>
                  <input value={shippingInfo.email} onChange={(e) => setShippingInfo(s => ({ ...s, email: e.target.value }))} placeholder="jane@example.com" />
                  <label>Street Address *</label>
                  <input value={shippingInfo.address} onChange={(e) => setShippingInfo(s => ({ ...s, address: e.target.value }))} placeholder="123 Main Street" />
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div><label>City *</label><input value={shippingInfo.city} onChange={(e) => setShippingInfo(s => ({ ...s, city: e.target.value }))} placeholder="Ottawa" /></div>
                    <div><label>Province *</label><input value={shippingInfo.province} onChange={(e) => setShippingInfo(s => ({ ...s, province: e.target.value }))} placeholder="Ontario" /></div>
                  </div>
                  <label>Postal Code *</label>
                  <input value={shippingInfo.postalCode} onChange={(e) => setShippingInfo(s => ({ ...s, postalCode: e.target.value }))} placeholder="K1A 0A1" />
                </div>
                <div className="buy-row">
                  <button className="secondary-btn" onClick={() => setCheckoutStep("cart")}>← Back</button>
                  <button className="primary-btn wide" onClick={() => setCheckoutStep("payment")}>Continue to Payment →</button>
                </div>
              </section>
